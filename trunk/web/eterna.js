...
function Eterna($E, eterna_debug, rootWebObj)
{
	...
	if (typeof Eterna._initialized == 'undefined')
	{
		...
		Eterna.prototype.ajaxVisit = function(url, param, logHistory)
		{
			...
			{
				var hCode = "$EH:" + this.id + "," + url;
				// 停止hash值变化的监控
				eg_cache.stopHashChangeEvent = true;
				location.href = "#" + encodeURIComponent(hCode);
			}
		}
...
function eterna_hashChange(event)
{
	...
	if (index != -1)
	{
		...
		if (hCode.indexOf("$EH:") == 0)
		{
			index = hCode.indexOf(',');
			var eId = hCode.substring(4, index);
			var _eterna = eg_cache.eternaCache[eId];
			if (_eterna != null)
			{
				_eterna.ajaxVisit(hCode.substring(index + 1), null, false);
			}
		}
	}
}

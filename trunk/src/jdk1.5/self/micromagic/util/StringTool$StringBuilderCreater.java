		...
		implements StringAppender, StringTool.StringAppenderCreater
{
	...
	StringTool$StringBuilderCreater()
	{
	}

	private StringTool$StringBuilderCreater(int initSize)
	{
		this.buf = new StringBuilder(initSize);
	}

	public StringAppender create(int initSize)
	{
		return new StringTool$StringBuilderCreater(initSize);
	}
